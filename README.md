...
### 🖥️ 现代化 GUI 可视化控制台 (推荐)
DetourFlow 配备了基于现代化深色玻璃拟态（Glassmorphism）设计的桌面控制面板：
- 🗂️ **拖拽极速注入**：支持将任意可执行文件（`.exe`）或快捷方式（`.lnk`）直接拖入主面板窗口，即刻完成 DLL 注入并代理拉起。
- 🕒 **最近运行历史与快捷拉起**：
  - **双栏视图**：左侧栏支持【活跃托管】与【运行历史】一键切换；
  - **秒级复用**：自动记录最近启动过的程序路径、最近拉起时间与使用频次，提供一键再次启动、定位目录与历史清理；
  - **顶部胶囊条**：拖拽区下方直观陈列最近使用过的快捷胶囊小组件，点击即刻代理启动。
- 💾 **自动持久化记忆**：自动保存上一次设置的 SOCKS5 主机、端口、IP 直连透传白名单，以及**历史托管应用列表**。重开软件后点击一键即可重新启动对应应用，无需重复找路径。
- 📊 **实时流量与活动流监控**：动态展示当前托管应用的流通道波形与连接状态，并支持一键在 Windows 资源管理器中定位源程序文件。
- 🔍 **专业级活动日志控制台**：
  - **分类过滤**：一键切换【全部】/【代理连接流 (Stream)】/【直连白名单 (Bypass)】/【异常报错 (Error)】。
  - **按需排查**：支持日志关键词快速模糊检索、暂停/自动滚动锁定切换。
  - **便捷运维**：提供一键复制过滤日志到剪贴板、一键清空日志面板等操作。

### 🛠️ 核心工作原理
1. **进程级沙箱分流**：无需配置 TUN/TAP 虚拟网卡，只针对注入的进程及其派生的整个进程链条做流量代理。
2. **工作目录智能保持**：启动器自动将目标所在目录设为工作目录，确保各类游戏与独立应用能正常加载本地配置文件与关联动态库。
3. **精准目标 PID 与日志对齐**：真实捕获目标子进程 PID，实现多进程日志流的精准跟踪与进程树优雅终止。
4. **IPv6 零延迟快速失败**：拦截所有外部 IPv6 连接并立即返回不可达，强制双栈程序（如 Chrome）瞬间回退到 IPv4 以走 SOCKS5 代理，避免了网络长连接超时挂起。
5. **沙箱自动规避**：拦截 `CreateProcess` 并在检测到 `chrome.exe` 时自动追加 `--no-sandbox` 启动参数，确保 Chrome 浏览器网络进程能顺利加载 DLL 和生效 Hook。
6. **回环代理过滤**：自动设置进程环境变量中的 `no_proxy`/`NO_PROXY` 绕过对本机 `127.0.0.1` 调试端口的劫持，避免因代理软件阻断回环通信导致的 AI 与浏览器交互超时。
7. **不污染标准输出**：日志仅输出到调试器（Debug String）和本地文件，不占用 `stdout`/`stderr`，保证 MCP 服务基于标准输出进行 JSON-RPC 通信的纯净度。
